...
def put_telugu_text(image, text, position, 
                    font_path='E:/Software/TeluguFonts/NotoSansTelugu-Regular.ttf', 
...
def display_transcriptions(frame, transcriptions, partial_transcription, x0, y0, dy, prev_length_difference, max_length=40):
    ...
    combined_transcription = ' '.join(transcriptions) + \
                             process_final_result(' '.join(partial_transcription))
    max_length = 25
    ...
    text_scaler = 3.0
    telugu_text_scaler = 2.0
    border = 2
    font_path = 'E:/Software/TeluguFonts/NotoSansTelugu-Bold.ttf'
    thickness = 5


    if not telugu:
        for i, line in enumerate(lines):
            y = y0 + i * dy * text_scaler
            (text_width, text_height), _ = cv2.getTextSize(line, cv2.FONT_HERSHEY_SIMPLEX, 0.5 * text_scaler, 1)
            adjusted_x0 = int(x0 - text_width // 2)
            cv2.putText(frame, line, (adjusted_x0 - border, int(y - border)), cv2.FONT_HERSHEY_SIMPLEX, .5 * text_scaler, (255, 255, 255), thickness, cv2.LINE_AA)
            cv2.putText(frame, line, (adjusted_x0 + border, int(y + border)), cv2.FONT_HERSHEY_SIMPLEX, .5 * text_scaler, (255, 255, 255), thickness, cv2.LINE_AA)
            cv2.putText(frame, line, (adjusted_x0 + border, int(y - border)), cv2.FONT_HERSHEY_SIMPLEX, .5 * text_scaler, (255, 255, 255), thickness, cv2.LINE_AA)
            cv2.putText(frame, line, (adjusted_x0 - border, int(y + border)), cv2.FONT_HERSHEY_SIMPLEX, .5 * text_scaler, (255, 255, 255), thickness, cv2.LINE_AA)
            cv2.putText(frame, line, (adjusted_x0 - border, int(y)), cv2.FONT_HERSHEY_SIMPLEX, .5 * text_scaler, (255, 255, 255), thickness, cv2.LINE_AA)
            cv2.putText(frame, line, (adjusted_x0 + border, int(y)), cv2.FONT_HERSHEY_SIMPLEX, .5 * text_scaler, (255, 255, 255), thickness, cv2.LINE_AA)
            cv2.putText(frame, line, (adjusted_x0, int(y - border)), cv2.FONT_HERSHEY_SIMPLEX, .5 * text_scaler, (255, 255, 255), thickness, cv2.LINE_AA)
            cv2.putText(frame, line, (adjusted_x0, int(y + border)), cv2.FONT_HERSHEY_SIMPLEX, .5 * text_scaler, (255, 255, 255), thickness, cv2.LINE_AA)
            cv2.putText(frame, line, (adjusted_x0, int(y)), cv2.FONT_HERSHEY_SIMPLEX, .5 * text_scaler, (0, 0, 255), thickness - 2, cv2.LINE_AA)
    else:
        for i, line in enumerate(lines):
            y = y0 + i * dy * text_scaler
            (text_width, text_height), _ = cv2.getTextSize(line, cv2.FONT_HERSHEY_SIMPLEX, 0.5 * text_scaler, 1)
            adjusted_x0 = int(x0 - 680 // 2)
            put_telugu_text(frame, line, (adjusted_x0 - border, int(y - border)), font_path, 20 * telugu_text_scaler, (255, 255, 255))
            put_telugu_text(frame, line, (adjusted_x0 + border, int(y + border)), font_path, 20 * telugu_text_scaler, (255, 255, 255))
            put_telugu_text(frame, line, (adjusted_x0 + border, int(y - border)), font_path, 20 * telugu_text_scaler, (255, 255, 255))
            put_telugu_text(frame, line, (adjusted_x0 - border, int(y + border)), font_path, 20 * telugu_text_scaler, (255, 255, 255))
            put_telugu_text(frame, line, (adjusted_x0 - border, int(y)), font_path, 20 * telugu_text_scaler, (255, 255, 255))
            put_telugu_text(frame, line, (adjusted_x0 + border, int(y)), font_path, 20 * telugu_text_scaler, (255, 255, 255))
            put_telugu_text(frame, line, (adjusted_x0, int(y - border)), font_path, 20 * telugu_text_scaler, (255, 255, 255))
            put_telugu_text(frame, line, (adjusted_x0, int(y + border)), font_path, 20 * telugu_text_scaler, (255, 255, 255))
            put_telugu_text(frame, line, (adjusted_x0, int(y)), font_path, 20 * telugu_text_scaler, (255, 0, 0))
...
def main():
    ...
    def extract_audio():
        process = (
            ffmpeg,
    out = cv2.VideoWriter('D:/study/mini project/transcriptions/output_with_subtitles.mp4', 
        ...
        audio_queue.put(None)
            frame = cv2.resize(frame, None, fx=test_scale_factor, 
                               fy=test_scale_factor)

            small_gray = cv2.resize(gray, None, fx=scale_factor, 
            faces = face_cascade.detectMultiScale(small_gray, scaleFactor=1.1, 
                                                  minNeighbors=5, minSize=(30, 30))
    recognize_speech_from_video(model, audio_queue, results)

                    cv2.rectangle(frame, (x * 4, y * 4), (x * 4 + w * 4, 
                                  y * 4 + h * 4), (255, 255, 255), 4)
    ...
    try:
        while running:
            ...
            faces = face_cascade.detectMultiScale(small_gray, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30))
                ...
                lines, length_difference = display_transcriptions(frame, transcriptions, 
                                                                 partial_transcription, x0, 
                                                                 y0 + chin_y + 30 + timer, dy, 
                                                                 prev_length_difference)
                if w > 100:
            ...
            if face_centers_x and face_centers_y:
                ...
                chin_y = compute_median(face_sizes_y)
    ...
    input_video = ffmpeg.input('D:/study/mini project/transcriptions/output_with_subtitles.mp4')
                    if result_type == 'full':
    ffmpeg.output(input_video, input_audio, 
                  'D:/study/mini project/transcriptions/final_output_with_audio.mp4', 
                  vcodec='copy', acodec='aac').run()
            ...
            dy = 20
            with lock:
                lines, length_difference = display_transcriptions(frame, transcriptions, partial_transcription, x0, y0 + chin_y + 30 + timer, dy, prev_length_difference)
